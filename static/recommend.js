...
function load_details(my_api_key, title) {
  fetch(`https://api.themoviedb.org/3/search/movie?api_key=${my_api_key}&query=${title}`)
...
function get_movie_details(movie_id, my_api_key, arr, movie_title) {
  fetch(`https://api.themoviedb.org/3/movie/${movie_id}?api_key=${my_api_key}`)
    .then(response => response.json())
    .then(movie_details => {
...
function get_individual_cast(movie_cast, my_api_key) {
  ...
  var promises = movie_cast.cast_ids.map(cast_id => {
    return fetch(`https://api.themoviedb.org/3/person/${cast_id}?api_key=${my_api_key}`)
      .then(response => response.json())
      .then(cast_details => {
...
function get_movie_cast(movie_id, my_api_key) {
  ...
  return fetch(`https://api.themoviedb.org/3/movie/${movie_id}/credits?api_key=${my_api_key}`)
    .then(response => response.json())
    .then(my_movie => {
      var top_cast = my_movie.cast.slice(0, 10);
      top_cast.forEach(my_cast => {
        cast_ids.push(my_cast.id);
        cast_names.push(my_cast.name);
        cast_chars.push(my_cast.character);
        cast_profiles.push("https://image.tmdb.org/t/p/original" + my_cast.profile_path);
      });
...
function get_movie_posters(arr, my_api_key) {
  ...
  var promises = arr.map(m => {
    return fetch(`https://api.themoviedb.org/3/search/movie?api_key=${my_api_key}&query=${m}`)
      .then(response => response.json())
      .then(m_data => {
  ...
  return Promise.all(promises).then(() => arr_poster_list);
}
